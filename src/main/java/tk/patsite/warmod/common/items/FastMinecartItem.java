...
public class FastMinecartItem extends Item {
    private static final DispenserBehavior DISPENSER_BEHAVIOR = new ItemDispenserBehavior() {
        ...
        public ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
            ...
            FastMinecartEntity fastMinecartEntity = new FastMinecartEntity(world, d, e + g, f);
            if (stack.hasCustomName()) {
                fastMinecartEntity.setCustomName(stack.getName());
            }

            world.spawnEntity(fastMinecartEntity);
            stack.decrement(1);
            return stack;
        }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!blockState.isIn(BlockTags.RAILS)) {
            ...
            if (!world.isClient) {
                ...
                FastMinecartEntity fastMinecartEntity = new FastMinecartEntity(world, (double)blockPos.getX() + 0.5, (double)blockPos.getY() + 0.0625 + d, (double)blockPos.getZ() + 0.5);
                if (itemStack.hasCustomName()) {
                    fastMinecartEntity.setCustomName(itemStack.getName());
                }
